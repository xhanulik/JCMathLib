...
#    put kelinci.jar, jcardsim-3.0.6.0.jar and bcprov-jdk18on-1.80.jar  into JCMathLib/libs/
...
export BC=/path/to/libs/bcprov-jdk18on-1.80.jar
...
#    WARNING: the Kelinci server requires separate terminal for launching and running
java -cp bin-instr:$JCARDSIM:$BC edu.cmu.sv.kelinci.Kelinci DifFuzzDriver @@

# 8. continue with setting up the AFL fuzzer in diffuzz_run_fuzzing.sh in separate terminal
